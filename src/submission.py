...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  # Forward pass
  z = outside_vectors.dot(center_word_vec)          # shape (V,)
  y_hat = softmax(z)                                # shape (V,)
  loss = -np.log(y_hat[outside_word_idx])

  # Backward pass
  y_hat_copy = np.copy(y_hat)
  y_hat_copy[outside_word_idx] -= 1.0               # y_hat - y

  grad_center_vec = outside_vectors.T.dot(y_hat_copy)   # shape (d,)
  grad_outside_vecs = y_hat_copy.reshape((-1, 1)).dot(
      center_word_vec.reshape((1, -1))
  )  # shape (V, d)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for outside_word in outside_words:
    ...
    l, grad_center, grad_outside = word2vec_loss_and_gradient(
        center_word_vec,
        outside_idx,
        outside_vectors,
        dataset
